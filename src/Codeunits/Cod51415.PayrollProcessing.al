...
Codeunit 51415 "Payroll Processing"//Number2
{
    ...
    procedure fnProcesspayroll_(strEmpCode: Code[20]; dtDOE: Date; curBasicPay: Decimal; blnPaysPaye: Boolean; blnPaysNssf: Boolean; blnPaysNhif: Boolean; SelectedPeriod: Date; dtOpenPeriod: Date; Membership: Text[30]; ReferenceNo: Text[30]; dtTermination: Date; blnGetsPAYERelief: Boolean; Dept: Code[20]; PayrollCode: Code[20])
    var
        strTableName: Text[50];
        curTransAmount: Decimal;
        curTransBalance: Decimal;
    ...
    begin
        ...
        if curBasicPay > 0 then begin
            ...
            ///>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.house Levy
            Curhouselevy := Round(curGrossPay * 1.5 / 100, 0.05, '<');
            if Curhouselevy > 0 then begin
                curTransAmount := Curhouselevy;
                strTransDescription := 'House Levy';
                TGroup := 'STATUTORIES';
                ...
                TSubGroupOrder := 4;
                fnUpdatePeriodTrans(strEmpCode, 'HL', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, NSSFEMPyee, Journalpostas::Credit, Journalpostingtype::"G/L Account", '',
                Coopparameters::none)
            end;
            // Message('Hse Levy amount of %1 posted CODE %2', Curhouselevy, strEmpCode);
            //Get the N.H.I.F amount for the month GBT
            curNhif_Base_Amount := 0;
            ...
            if blnPaysNhif then begin
                ...
                // Message('SHIF CHARDGED OF %1', curNHIF);
            end;
            //Get the Defined contribution to post based on the Max Def contrb allowed   ****************All Defined Contributions not included
            curDefinedContrib := curNSSF + Curhouselevy + curNHIF; //(curNSSF + curPensionStaff + curNonTaxable) - curMorgageReliefAmount
                                                                   // curDefinedContrib:= fnGetEmployeeNSSF
            ...
            if blnPaysNhif then begin
                ...
                //curNHIF:=curNHIF*0.15;
                curNHIF := (2.75 * curGrossPay) / 100;
                // Message('n/SHIF RELIEF @2 IS %1', CURNHIF);
                    ...
                    if prTransactionCodes.Find('-') then begin
                        ...
                        if (prTransactionCodes."Special Transaction" = prTransactionCodes."special transaction"::"Staff Loan") and
                           (prTransactionCodes."Repayment Method" = prTransactionCodes."repayment method"::Reducing) then begin
                            ...
                            if (FnLoanInterestExempted(prEmployeeTransactions."Loan Number") = false) then
                                curLoanInt := fnCalcLoanInterest(strEmpCode, prEmployeeTransactions."Transaction Code", FnGetInterestRate(prEmployeeTransactions."Transaction Code"), prTransactionCodes."Repayment Method",
                                   prEmployeeTransactions.Amount, prEmployeeTransactions.Balance, SelectedPeriod, false/* , prEmployeeTransactions."Loan Number" */);
                            curTransAmount := curLoanInt;
                            //for Reducing mk
                            prEmployeeTransactions."Interest Charged" := curTransAmount;
                        ...
                        IF (prTransactionCodes."Special Transaction" = prTransactionCodes."Special Transaction"::"Staff Loan") AND
                            ...
                            curTransAmount := 0;
                            curLoanInt := 0;
                            // Retrieve the outstanding loan balance from the Loans Register
                            LoanRegister.RESET;
                            LoanRegister.SETRANGE(LoanRegister."Loan  No.", prEmployeeTransactions."Loan Number");
                            ...
                            // Debug message to verify the fetched balance
                            // MESSAGE('Updated Loan Balance from Register: %1 for Loan No: %2', prEmployeeTransactions.Balance, prEmployeeTransactions."Loan Number");

                            // Now perform interest calculation with the updated balance

                            // Debug message for Loan Balance before interest calculation
                            // MESSAGE('Loan Balance Before Interest Calculation: %1, loan number is %2', prEmployeeTransactions.Balance, prEmployeeTransactions."Loan Number");
                            ...
                            // Debug message for calculated Loan Interest
                            //MESSAGE('Calculated Loan Interest: %1', curLoanInt);

                            IF curLoanInt > 0 THEN BEGIN
                                curTransAmount := curLoanInt;
                                curTotalDeductions := curTotalDeductions + curTransAmount; // Sum-up all deductions
                                curTransBalance := 0;

                                // Create a new transaction for Interest Posting
                                strTransCode := prEmployeeTransactions."Transaction Code" + '-INT';
                                strTransDescription := prEmployeeTransactions."Transaction Name" + ' Interest';
                                TGroup := 'DEDUCTIONS';
                                TGroupOrder := 8;
                                ...
                                // Store the Interest in the Loan Record
                                prEmployeeTransactions."Interest Charged" := curTransAmount;
                                prEmployeeTransactions.MODIFY;
                            END ELSE BEGIN
                                //MESSAGE('Skipping interest posting as calculated interest is 0.');
                            END;

                            // Adjust the principal repayment amount
                            curTransAmount := prEmployeeTransactions."Amtzd Loan Repay Amt" - curLoanInt;

                            // Debug message for Amortized Loan Repayment Amount
                            //MESSAGE('Amortized Loan Repayment Amount: %1', prEmployeeTransactions."Amtzd Loan Repay Amt");

                            // Update the loan repayment amount
                            prEmployeeTransactions.Amount := curTransAmount;
                            prEmployeeTransactions.MODIFY;

                            // Final debug message for confirmation
                            // MESSAGE('Final Loan Repayment Amount: %1 |Description -> %5| Interest Amount: %2 | Original Amount: %3 | Interest Rate: %4',
                            //         curTransAmount, curLoanInt, prEmployeeTransactions."Original Amount", prTransactionCodes."Interest Rate", strTransDescription);
                        END;
            ...
            //**************END OF EMPLOYEE HOUSING LEVY CALCULATION***************//
            //Get the Gross taxable amount
            //>GrossTaxable = Gross + Benefits + nValueofQuarters  ******Confirm CurValueofQuaters
            curGrossTaxable := curGrossPay + curBenefits + curValueOfQuarters;
            ...
            // IF EmployeeP.FIND('-') THEN BEGIN
            //
            //     // CurrInsuranceRel := fnGetEmployeeNHIF(curNhif_Base_Amount)*(0.15);
            //      CurrInsuranceRel:=fnGetEmployeeNHIF(curNhif_Base_Amount);
            //
            //      //Insert Insurance Relief
            //      curTransAmount := CurrInsuranceRel;
            //      strTransDescription := 'Insurance Relief';
            ...
            if blnPaysNhif then begin
                ...
                fnUpdatePeriodTrans(strEmpCode, 'NHIF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                 curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept,
                 NHIFEMPyee, Journalpostas::Credit, Journalpostingtype::"G/L Account", '', Coopparameters::NHIF);
                // Message('SHIF | %1', curNHIF);

            end;
            //SHIF
            // MESSAGE('BASE IS %1', curNhif_Base_Amount);
            // IF blnPaysNhif THEN BEGIN
            //  //curNHIF:=450;
            // curNHIF:= fnGetEmployeeNHIF(curNhif_Base_Amount);
            // //curNHIF:= fnGetEmployeeNHIF(curGrossPay)*2.75/100;
            // //curNHIF:=curGrossPay*2.75/100;
            //  curTransAmount :=curNHIF;
            //  strTransDescription := 'S.H.I.F';
            ...
            if EmployeeP.Find('-') then begin
                ...
                CurrInsuranceRel := fnGetEmployeeNHIF(curNhif_Base_Amount) * (0.15);
                //CurrInsuranceRel:=curNHIF*(VitalSetup.NHIFRelief/100);
            ...
            if EmployeeP.Find('-') then begin
                ...
                CurrInsuranceRel := fnGetEmployeeNHIF(curNhif_Base_Amount) * (0.15);
                ////CurrPremiumRel:=EmployeeP."Insurance Premium"*0.15;

                //Insert Insurance Relief
                curTransAmount := CurrInsuranceRel;
                strTransDescription := 'Premium Insurance Relief';
                TGroup := 'TAX CALCULATIONS';
                TGroupOrder := 6;
                TSubGroupOrder := 8;
                //         fnUpdatePeriodTrans(strEmpCode, 'INSPREL', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                //    curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', Journalpostas::" ", Journalpostingtype::" ", '',
                //    Coopparameters::none);
            ...
            if EmployeeP.Find('-') then begin
                ...
                CurrInsuranceRel := fnGetEmployeeNHIF(curNhif_Base_Amount) * (0.15);
                //  AHLRelief:=HSFUND*0.15;

                //Insert Insurance Relief
                curTransAmount := CurrInsuranceRel;
                strTransDescription := 'A.H.L Relief';
                TGroup := 'TAX CALCULATIONS';
                TGroupOrder := 6;
                TSubGroupOrder := 8;
                //         fnUpdatePeriodTrans(strEmpCode, 'AHLRelf', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                //    curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', Journalpostas::" ", Journalpostingtype::" ", '',
                //    Coopparameters::none);
            ...
            IF curPensionStaff > curMaxPensionContrib THEN
                curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curMaxPensionContrib + curOOI + curHOSP + curNonTaxable + AHLRelief)

            ELSE
                curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curPensionStaff + curOOI + curHOSP + curNonTaxable + AHLRelief);
            //Taxable Benefit
            txBenefitAmt := 0;
            IF EmpSalary.GET(strEmpCode) THEN BEGIN
                IF EmpSalary."Pays NSSF" = FALSE THEN BEGIN
                    IF fnCheckPaysPension(strEmpCode, SelectedPeriod) = TRUE THEN BEGIN
                        IF (EmpSalary."Basic Pay" * 0.1) > 20000 THEN BEGIN
                            txBenefitAmt := EmpSalary."Basic Pay" * 0.2;
                        END ELSE BEGIN
                            txBenefitAmt := ((EmpSalary."Basic Pay" * 0.1) + (EmpSalary."Basic Pay" * 0.1)) - 20000;
                            IF txBenefitAmt < 0 THEN
                                txBenefitAmt := 0;
                        END;
                        strTransDescription := 'Taxable Pension';
            ...
            //Get the Tax charged for the month
            curTaxablePay := curTaxablePay + txBenefitAmt;
            curTaxCharged := fnGetEmployeePaye(curTaxablePay) - (curInsuranceReliefAmount + curReliefPersonal + CurrInsuranceRel);//factor in the 15% nhif relief
            ...
            // BEGIN
            // //curNHIF:= fnGetEmployeeNHIF(curNhif_Base_Amount);
            // curNHIFReliefInsurance := curNHIF*0.15; //*****Get curUnusedRelief
            // MESSAGE('nhif', FORMAT(curNHIF));
            // curTransAmount := curNHIFReliefInsurance;
            ...
            IF prEmployeeTransactions.FIND('-') THEN BEGIN
                        ...
                        IF (prTransactionCodes."Special Transaction" = prTransactionCodes."Special Transaction"::"Staff Loan") AND
                           (prTransactionCodes."Repayment Method" = prTransactionCodes."Repayment Method"::Amortized) THEN BEGIN
                            ...
                            prTransactionCodes."Interest Rate", prTransactionCodes."Repayment Method",
                              prEmployeeTransactions."Original Amount", prEmployeeTransactions.Balance, SelectedPeriod, FALSE/* , prEmployeeTransactions."Loan Number" */);
                            //Post the Interest
                            //MESSAGE('Loanno %1|curLoanInt %2|prEmployeeTransactions."Original Amount" %3,Loan Balance %4', prEmployeeTransactions."Loan Number", curLoanInt, prEmployeeTransactions."Original Amount", prEmployeeTransactions.Balance);
                            //IF (curLoanInt<>0) THEN BEGIN
                            curTransAmount := curLoanInt;
                            ...
                            prEmployeeTransactions.Amount := curTransAmount;
                            prEmployeeTransactions.MODIFY;
                            // Message('Amortized @2 here %1 LoanInt, %2 transaction Desc, CurTransAmount of %3', curLoanInt, strTransCode, curTransAmount);
                        END;
                        //Loan Calculation Amortized
                ...
                // Message('%1 posted total of %2', strTransDescription, curTotalDeductions);
