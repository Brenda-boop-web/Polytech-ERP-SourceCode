    ...
    begin
        ...
        IF Cust.get(MemberNo) then begin
            if Confirm('Proceed With Account Closure ?', false) = false then begin
                ...
                //Ledger Fee Charges
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(TemplateName, BatchName, Doc_No, LineNo, GenJournalLine."Transaction Type"::" ", GenJournalLine."Account Type"::"G/L Account", GenSetUp."Ledger Fee Account", Rec."Posting Date", (Rec."Ledger Fee" - (Rec."Ledger Fee" * GenSetUp."Excise Duty(%)" / 100)) * -1, 'BOSA', MemberNo, 'Ledger fees charge for exit', '');
                RunningBal := RunningBal - Rec."Ledger Fee";

                //Excise Duty 15% Charges
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(TemplateName, BatchName, Doc_No, LineNo, GenJournalLine."Transaction Type"::" ", GenJournalLine."Account Type"::"G/L Account", GenSetUp."Excise Duty Account", Rec."Posting Date", (Rec."Ledger Fee" * GenSetUp."Excise Duty(%)" / 100) * -1, 'BOSA', MemberNo, 'Excise Duty charge for exit', '');
                //RunningBal := RunningBal - Rec."Ledger Fee";

                //Credit Bank
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(TemplateName, BatchName, Doc_No, LineNo, GenJournalLine."Transaction Type"::" ", GenJournalLine."Account Type"::"G/L Account", Rec."Paying Bank", Rec."Posting Date", RunningBal * -1, 'BOSA', MemberNo, 'credit Refund Deposits Account Exit', '');
            end;
